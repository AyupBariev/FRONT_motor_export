                ...
                <div className="CardOfferBody__leftColumn">
                    <ul className="CardInfo__list-MZpc1">
                        {Object.entries(carData).map(([key, value]) => {
                            ...
                            if (info) {
                                return <li className="CardInfoRow">
                                    <span className="CardInfoRow__cell">{info.label}</span><span className="CardInfoRow__cell">{value}</span>
                                </li>;
                            }
                            return null;
                        })}
                    </ul>
                    <a  className="linkMessage" href={"https://api.whatsapp.com/send/?phone=971559299569&text=" + encodeURIComponent('Hello! I want to buy the car ' + process.env.REACT_APP_BACKEND_MOTOR_EXPORT + '/api/v1/car/' + guid) + "&type=phone_number&app_absent=0"}><img
